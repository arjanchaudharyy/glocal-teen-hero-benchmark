...
**On the vocabulary-overlap finding.** This is, in retrospect, an obvious problem: writing gold queries by paraphrasing your own memory of what a document says will tend to reproduce that document's words, especially for a single process (here, Claude, in the same session that wrote the corpus) working from a corpus it built. The fix (write queries as if you did not know the target vocabulary, then verify labels against the actual text rather than memory) is cheap and worth doing before, not after, reporting numbers.
